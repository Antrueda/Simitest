...
Route::get('fi/ficomposicionfamiliar', function (Request $request) { 
	if (!$request->ajax()) 
	return redirect('/');
	$nnajidxx=FiDatosBasico::where('sis_nnaj_id',$request->sis_nnaj_id)->first();
	return datatables()
		->eloquent(FiComposicionFami::where('activo', 1)->where('fi_nucleo_familiar_id', $nnajidxx->fi_nucleo_familiar_id))
...
Route::get('fi/firedapoyoantecedente', function (Request $request) { 
	if (!$request->ajax()) return redirect('/');
	$redapoyo=FiRedApoyoAntecedente::select('fi_red_apoyo_antecedentes.id',
	'sis_entidads.nombre',
...
Route::get('fi/firedapoyoactual', function (Request $request) { 
	if (!$request->ajax()) return redirect('/');
	//   
	$actualxx=FiRedApoyoActual::select('fi_red_apoyo_actuals.id',
...
Route::get('fi/fijrfamiliar', function (Request $request) {
	if (!$request->ajax()) return redirect('/');
	return datatables()
		->eloquent(FiJusticiaRestaurativa::
		select('fi_proceso_familias.id',
		'fi_composicion_famis.sis_nnaj_id',
		'fi_justicia_restaurativas.sis_nnaj_id',
		'fi_proceso_familias.activo',
		'fi_composicion_famis.s_primer_nombre', 'fi_composicion_famis.s_segundo_nombre', 'fi_composicion_famis.s_primer_apellido','fi_composicion_famis.s_segundo_apellido', 
...
Route::get('fi/fisaludenfermedad', function (Request $request) {
	...
	return datatables()
		->eloquent(FiSalud::
		select('fi_enfermedades_familias.id',
		'fi_composicion_famis.sis_nnaj_id',
...
Route::get('fi/fisustanciaconsumida', function (Request $request) {
	if (!$request->ajax()) return redirect('/');
	return datatables()
		->eloquent(FiConsumoSpa::
		select('fi_sustancia_consumidas.id',
...
Route::get('is/intervencion', function (Request $request) { 
	if (!$request->ajax()) return redirect('/');
	//   
	$actualxx=IsDatosBasico::select('is_datos_basicos.id',
	'is_datos_basicos.sis_nnaj_id', 'is_datos_basicos.sis_nnaj_id',
...
Route::get('fos/fichaobservacion', function (Request $request) { 
	if (!$request->ajax()) return redirect('/');
	//   
	$actualxx=FosDatosBasico::select('fos_datos_basicos.id',
	'fos_datos_basicos.sis_nnaj_id', 'fos_datos_basicos.sis_nnaj_id',
