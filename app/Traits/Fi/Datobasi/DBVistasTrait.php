...
trait DBVistasTrait
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->s_documento == "111111111111") {
            ...
            $respuest = FiDatosBasico::orderBy('id', 'ASC')
                ->whereBetween('id', [  $minimoxx,$maximoxx])
                ->get();
            $modeloxx = "FiDatosBasico";
            $posterio = 0;
            $fidatosx=8249;
            // $ficha = '$ficha=[<br>';
            foreach ($respuest as $key => $value) {
                ...
                if ($diferenc > 1) {
                    ...
                    //    $value->s_documento=str_replace(['-'],'',date('Y-m-d'));
                ...
                $value->sis_nnaj_id = $original;
                // // $value->s_documento=$document;
