...
use App\Models\Tema;
use App\Traits\Acciones\Individuales\Emprender\Egreso\Redes\CrudTrait;
use App\Traits\Acciones\Individuales\Emprender\Egreso\Redes\ParametrizarTrait;
use App\Traits\Acciones\Individuales\Emprender\Egreso\Redes\VistasTrait;
...
class EgresoRedesApoyoController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->opciones['permisox'] = 'egresoredes';
        $this->opciones['routxxxx'] = 'egresoredes';
        $this->getOpciones();
    ...
    public function Redactual($id){
        $dato = SEgreso::where('id',$id)->first();
        $tipo = Tema::combo(23, true, false);
        return view( $this->opciones['rutacarp'].$this->opciones['carpetax'].'.Formulario.formactual', compact('dato','tipo'));
   }


   public function CreaRedactual(Request $request,$id){
    ddd($request);
    $detalle = $request->input("desistir");
    $request->request->add(['egreso_id'=> $id]);
    $modelo = EgreApoyo::create($request->all());
    
    return redirect()->route($this->opciones['routxxxx'] . '.editar')->with('info', 'Registro actualizado con éxito');
