        ...
        $objetoxx = DB::transaction(function () use ($dataxxxx) {
            ...
            if($dataxxxx['requestx']->victimas) {
                foreach ($dataxxxx['requestx']->victimas as $d) {
                    $dataxxxx['modeloxx']->victimas()->attach($d, ['user_crea_id' => Auth::user()->id, 'user_edita_id' => Auth::user()->id]);
                }
            }
            $dataxxxx['modeloxx']->riesgos()->detach();
            if ($dataxxxx['requestx']->riesgos) {
                foreach ($dataxxxx['requestx']->riesgos as $d) {
                    $dataxxxx['modeloxx']->riesgos()->attach($d, ['user_crea_id' => Auth::user()->id, 'user_edita_id' => Auth::user()->id]);
                }
            }
