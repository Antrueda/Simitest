<div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        ...
        @else
        @if(auth()->user()->can($requestx->routexxx[0] . '-activar'))
        <div class="dropdown-item">
            <a class="btn btn-sm btn-success " href="{{ route($requestx->routexxx[0].'.activarx', [$queryxxx->id]) }}">ACTIVAR</a>
        </div>
        @endif
        @endif
        @if(auth()->user()->can( $requestx->routexxx[1].'-leer'))
        <div class="dropdown-item">
            <a class="btn btn-sm btn-primary " href="{{ route($requestx->routexxx[1], $queryxxx->id) }}">ÍTEMS A EVALUAR</a>
        </div>
        @endif
