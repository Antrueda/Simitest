    ...
    <div class="col-md-6">
        {{ Form::label('tconvive', 'Tiempo de convivencia', ['class' => 'control-label col-form-label-sm']) }}
            <table class="table" id="tconvive">
                ...
                <tbody>
                    <tr>
                        <td>{{ Form::number('dia', null, ['class' => $errors->first('dia') ? 'form-control form-control-sm is-invalid' : 'form-control form-control-sm', 'placeholder' => 'Día(s)', 'min' => '0', 'max' => '99',"onkeypress" => "return soloNumeros(event);"]) }}
                            @if($errors->has('dia'))
                            <div class="invalid-feedback d-block">
                                {{ $errors->first('dia') }}
                            </div>
