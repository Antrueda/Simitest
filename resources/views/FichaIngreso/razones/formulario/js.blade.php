...
//MASCARA DOCUMENTOS
  $(function () {
    ...
    var f_cargos = function (dataxxxx) {
      $.ajax({
        url: "{{ route('fi.razones.cargos',$todoxxxx['nnajregi'])}}",
        type: 'GET',
    ...
    @if (old('userd_id')!=null))
      f_cargos({dataxxxx: {valuexxx: {{old('userd_id')}}, campoxxx: 'userd_id', selected: '{{old("sis_depend_id")}}'});
    @endif
    @if (old('userr_id')!=null))
      f_cargos({dataxxxx: {valuexxx: {{old('userr_id')}}, campoxxx: 'userr_id', selected: '{{old("sis_depenr_id")}}'});
    @endif
