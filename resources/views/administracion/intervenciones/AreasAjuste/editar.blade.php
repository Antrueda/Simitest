...
@section('content')
    ...
    <form method='post' action='{{ route($rutaxxxx . '.update', ['atencion' => Route::current()->parameters['atencion'],'intarea' =>  $parametro->id]) }}'>
        @csrf
        @method('PUT')
        @include('administracion.intervenciones.AreasAjuste.botones')
                ...
                <div class="form-group">
                    ...
                    <div class="form-select-shower">
                        <select name="parametro_id" id="parametro_id" class="form-control">
                            @foreach ($areasAjuste as $areaajus)
                                <option value="{{ $areaajus->id }}" @if ($parametro->parametro_h_id == $areaajus->id)
                                    selected
                            @endif >{{ $areaajus->nombre }}</option>
                            @endforeach
                ...
                <div class="form-group">
                    ...
                    <select name="sis_esta_id" id="sis_esta_id" class="form-control">
                        @foreach ($estadoxx as $estado)
                            <option value="{{ $estado->id }}" @if ($parametro->sis_esta_id == $estado->id)
                                selected
