<div class="form-group row{{ $errors->first('nombre') ? ' is-invalid' : '' }}">
	...
	<div class="col-sm-10">
		...
		@else
			{{ Form::text('nombre', null, ['class' => $errors->first('nombre') ? 'form-control is-invalid' : 'form-control', 'placeholder' => 'nombre de la actividad', 'maxlength' => '120', 'autofocus','onkeyup' => 'javascript:this.value=this.value.toUpperCase();', 'style' => 'text-transform:uppercase;']) }}
		@endif
	</div>
...
<div class="form-group row{{ $errors->first('sis_docfuen_id') ? ' is-invalid' : '' }}">
	{{ Form::label('sis_docfuen_id', 'Documento fuente:', ['class' => 'col-sm-2 col-form-label']) }}
	<div class="col-sm-10">
		@if($accion == 'Ver')
