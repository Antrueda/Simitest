    ...
    <tbody>
        @foreach ($fiDatosBasicos as $fiDatosBasico)
            <tr>
                ...
                @if (!is_null($fiDatosBasico->sis_nnaj->fi_actividadestls))
                    {{-- 8.1 ¿Cuánto tiempo al día permanece en la calle? --}}
                    <td>{{ $fiDatosBasico->sis_nnaj->fi_actividadestls->i_horas_permanencia_calle ?? 'Sin dato' }}</td>
                    {{-- 8.2 ¿Cuántos días a la semana? --}}
                    <td>{{ $fiDatosBasico->sis_nnaj->fi_actividadestls->i_dias_permanencia_calle ?? 'Sin dato' }}</td>
                    {{-- 8.4 ¿Pertecene a algún grupo, parche u organización? --}}
                    <td>{{ $fiDatosBasico->sis_nnaj->fi_actividadestls->i_prm_pertenece_parche->nombre ?? 'Sin dato' }}</td>
                    {{-- 8.5 ¿Tiene acceso a recreación? --}}
                    <td>{{ $fiDatosBasico->sis_nnaj->fi_actividadestls->i_prm_acceso_recreacion->nombre ?? 'Sin dato' }}</td>
                    {{-- 8.6 ¿Tiene prácticas religiosas? --}}
                    <td>{{ $fiDatosBasico->sis_nnaj->fi_actividadestls->i_prm_practica_religiosa->nombre ?? 'Sin dato' }}</td>
                    {{-- 8.7 ¿Cuál religión practica? --}}
                    <td>{{ $fiDatosBasico->sis_nnaj->fi_actividadestls->i_prm_religion_practica->nombre ?? 'Sin dato' }}</td>
                @else
                    <td>Sin evaluar</td>
                    <td>Sin evaluar</td>
                    <td>Sin evaluar</td>
